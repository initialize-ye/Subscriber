from nonebot import on_command
from nonebot.adapters.onebot.v11 import Message
from nonebot.rule import to_me

help_matcher = on_command("help", aliases={"帮助", "?"}, rule=to_me(), priority=8, block=True)

HELP_TEXT = """━━━ 订阅管理 ━━━
  添加订阅 - 交互式添加 B站/微博 订阅
  查询订阅 - 查看所有订阅
  订阅状态 - 查看订阅详情（分类、标签、Cookie）
  编辑订阅 - 修改已有订阅的分类和标签
  删除订阅 - 删除订阅

━━━ Cookie 管理 ━━━
  添加cookie - 添加 Cookie（也可用：添加Cookie）
  查看cookie / 我的cookie - 查看已添加的 Cookie
  关联cookie - 为订阅关联 Cookie
  取消关联cookie - 取消 Cookie 关联
  删除cookie - 删除 Cookie

━━━ 群管理 ━━━
  群管理 - 远程管理指定群的订阅（私聊使用）

━━━ 后台 ━━━
  管理后台 / 后台管理 - 生成管理后台地址（私聊使用）

━━━ 其他 ━━━
  help / 帮助 / ? - 显示本帮助

支持平台：B站动态、B站直播、B站番剧、微博
提示：微博订阅需要 Cookie 才能正常使用"""


@help_matcher.handle()
async def handle_help():
    await help_matcher.finish(Message(HELP_TEXT))
